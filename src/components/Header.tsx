      ...
      <nav className='hidden md:flex md:items-center md:justify-between md:px-40'>
        <div className='flex items-center gap-8'>
          {
            menuItems.map((item, index) => {
              return <a
                key={index}
                href={ item.link }
                className='cursor-pointer border-b-2 border-transparent hover:border-amber-400'
              >
                {item.label}
              </a>
            })
        ...
        <div>
          <ul className='flex items-center gap-5'>
            <a href='https://github.com/lucianoleyriaa' target='_blank' className='text-xl cursor-pointer'>
